...
%OrangeLinear sums 
SumOLCol = sum(handles.FinalPictureOrangeLinear);
SumOLRow = sum(SumOLCol);
OrangeLnrGrnChnlSum = SumOLRow(:,:,2);

%GreenLinear sums
SumGLCol = sum(handles.FinalPictureGreenLinear);
SumGLRow = sum(SumGLCol);
GreenLnrGrnChnlSum = SumGLRow(:,:,2);

%BlueLinear sums
SumBLCol = sum(handles.FinalPictureBlueLinear);
SumBLRow = sum(SumBLCol);
BlueLnrGrnChnlSum = SumBLRow(:,:,2);

%BlueCircular sums
SumBCCol = sum(handles.FinalPictureBlueCircular);
